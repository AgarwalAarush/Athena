        ...
        ZStack {
            if conversationService.currentConversation == nil {
                ...
                VStack(spacing: 0) {
                    ...
                    if let status = appViewModel.messagingStatus {
                        StatusBannerView(message: status, isSuccess: !status.contains("Failed")) {
                            appViewModel.clearMessagingStatus()
                        }
                        .padding(.horizontal)
                        .padding(.bottom, 8)
                    }
...
struct StatusBannerView: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 16)
        .padding(.vertical, 8)
        .background((isSuccess ? Color.green : Color.orange).opacity(0.1))
    }
}
